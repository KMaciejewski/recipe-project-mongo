...
public class IngredientControllerTest {
    ...
    void findAllByRecipeId() {
        ...
        when(ingredientService.findAllByRecipeId(anyLong())).thenReturn(ingredients);

        mockMvc.perform(get(buildUrl(RECIPE, ONE.toString(), INGREDIENTS)))
    ...
    void findRecipeIngredient() {
        ...
        mockMvc.perform(get(buildUrl(RECIPE, ONE.toString(), INGREDIENTS, ONE.toString())))
                .andExpect(status().isOk())
    ...
    void update() {
        ...
        mockMvc.perform(get(buildUrl(RECIPE, ONE.toString(), INGREDIENTS, ONE.toString(), "update")))
                .andExpect(status().isOk())
    ...
    void saveOrUpdate() {
        ...
        mockMvc.perform(post(buildUrl(RECIPE, ONE.toString(), INGREDIENTS)))
    ...
    void create() {
        ...
        mockMvc.perform(get(buildUrl(RECIPE, ONE.toString(), INGREDIENTS, "new")))
                .andExpect(status().isOk())
                .andExpect(view().name(INGREDIENTS + "/form"))
    ...
    @Test
    void delete() {
        mockMvc.perform(get(buildUrl(RECIPE, ONE.toString(), INGREDIENTS, ONE.toString(), "delete")))
                .andExpect(status().is3xxRedirection())
